...
**Target:** Jupyter Notebook (`notebooks/<your_name>/00_setup_test.ipynb`)  
**Prompt:**
```text
Act as a Senior AWS ML Engineer. Write a self-contained Python script for a Jupyter notebook cell that validates our environment for the Amazon ML Challenge 2026.
Requirements:
1. Check if required libraries ('pandas', 'numpy', 'rapidfuzz', 'lightgbm', 'pyarrow', 'boto3', 's3fs') are installed; print pip install commands if missing.
2. Test read and write permissions to our S3 bucket 's3://<INSERT_YOUR_BUCKET_NAME>/' by creating a tiny dummy DataFrame, saving it as parquet directly to S3, and reading it back.
...
**Prompt:**
```text
Act as an Expert Data Scientist. Write clean, modular Jupyter notebook code to perform Exploratory Data Analysis (EDA) on a 50,000-row sample of the Amazon ML Challenge dataset from S3.
Requirements:
1. Read the first 50,000 rows from 'dataset/train/train_source1.tsv' using pandas with sep="\t".
...
4. Keep memory footprint under 500 MB. Provide concise explanations of what the findings mean for feature engineering.
```

---

## Stage 2 (Team A): Text Normalization & Inverted Index Blocking
...
1. 'clean_text(text)': Unicode NFKD normalization, lowercase, standardization of legal suffixes ('pvt', 'ltd', 'corp', 'inc', 'llc', 'llp', 'co'), street abbreviations ('rd', 'st', 'ave', 'blvd'), and handling of multilingual characters (Hindi and French accents).
...
3. Self-Critique: Review your code before providing it. Ensure it handles missing values (NaNs), does not crash on empty strings, and handles the French test set seamlessly.
```

---

## Stage 3 (Team A): Feature Extraction & Parquet Export to S3
...
4. Export 'train_candidates.parquet' and 'val_candidates.parquet' directly to 's3://<BUCKET_NAME>/processed/full/'.
...
1. Reads 'dataset/test/test_source1.tsv', 'test_source2.tsv', and 'test_source3.tsv'.
...
1. Load 'train_candidates.parquet' and 'val_candidates.parquet' from S3.
...
5. Output feature importance to show which similarities matter most.
```
...
4. Apply this optimal threshold to 'test_candidates.parquet'.
5. Generate:
...
3. Check that 'code/business_entity_resolution/requirements.txt' pins exact versions.
4. Create the zip archive without extraneous '__pycache__' or hidden files.
5. Print the total file size and directory structure of the created zip archive.
```
...
Context:
- Machine: AWS SageMaker Notebook (ml.m5.xlarge with 16GB RAM)
